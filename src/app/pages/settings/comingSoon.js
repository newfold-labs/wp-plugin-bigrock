...
const ComingSoon = () => {
	...
	const getComingSoonNoticeTitle = () => {
		return comingSoon
			? __( 'Coming soon activated', 'wp-plugin-bigrock' )
			: __( 'Coming soon deactivated', 'wp-plugin-bigrock' );
	};
	...
	const getComingSoonSectionTitle = () => {
		const getStatus = () => {
			return comingSoon ? (
				<span className="nfd-text-[#e10001] coming-soon-status">
					{ __( 'Not Live', 'wp-plugin-bigrock' ) }
				</span>
			) : (
				<span className="nfd-text-[#008112] coming-soon-status">
					{ __( 'Live', 'wp-plugin-bigrock' ) }
				</span>
			);
		};
		...
		return (
			<span>
				{ __( 'Site Status', 'wp-plugin-bigrock' ) }: { getStatus() }
			</span>
		);
	};
		...
		>
			<div className="nfd-flex nfd-flex-col nfd-gap-6">
				<ToggleField
					...
					label={__('Coming soon page', 'wp-plugin-bigrock')}
					description={__(
				...
				{comingSoon &&
					<Alert variant="info">
						{__('Your website is currently displaying a "Coming Soon" page.', 'wp-plugin-bigrock')}
					</Alert>
				}
