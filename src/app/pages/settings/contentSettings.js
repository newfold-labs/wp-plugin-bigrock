...
const ContentRevisions = ({ setError, notify }) => {
	...
	const contentRevisionsNoticeTitle = () => {
		return (
			__('Post revision setting saved ', 'wp-plugin-bigrock')
		);
	};

	const contentRevisionsNoticeText = () => {
		return (
			__('Posts will save  ', 'wp-plugin-bigrock') +
			contentRevisions +
			_n(' revision.', ' revisions.', parseInt(contentRevisions), 'wp-plugin-bigrock')
		);
	};

	const contentRevisionsDescriptionText = () => {
		return (
			__('Saving drafts and updating published content creates revisions. Make changes with confidence, knowing you can take ', 'wp-plugin-bigrock') +
			contentRevisions +
			_n(' step back.', ' steps back.', parseInt(contentRevisions), 'wp-plugin-bigrock')
		);
	};
	...
	return (
		<SelectField
			...
			label={__('Number of revisions posts can save ', 'wp-plugin-bigrock')}
...
const EmptyTrash = ({ setError, notify }) => {
	...
	const emptyTrashNoticeTitle = () => {
		return (
			__('Trash setting saved ', 'wp-plugin-bigrock')
		);
	};

	const emptyTrashNoticeText = () => {
		return (
			__('The trash will automatically empty every ', 'wp-plugin-bigrock') +
			numTrashWeeks +
			_n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-bigrock' )
		);
	};
	...
	return (
		<SelectField
			...
			label={__('Trash emptying frequency ', 'wp-plugin-bigrock')}
			description={
			__('The trash will automatically empty every ', 'wp-plugin-bigrock') +
			numTrashWeeks +
			 _n( ' week.', ' weeks.', parseInt(numTrashWeeks), 'wp-plugin-bigrock' )
...
const ContentSettings = () => {
	...
	return (
		<Container.SettingsField
			title={__('Content Options', 'wp-plugin-bigrock')}
			description={__('Controls for content revisions and how often to empty the trash.', 'wp-plugin-bigrock')}
		>
