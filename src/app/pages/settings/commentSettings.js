...
const OldPostsComments = ({ setError, notify }) => {
	...
	const disableCommentsNoticeTitle = () => {
		return disableCommentsOldPosts
			? __('Disabled old post comments', 'wp-plugin-bigrock')
			: __('Enabled old post comments', 'wp-plugin-bigrock');
	};

	const disableCommentsNoticeText = () => {
		return disableCommentsOldPosts
			? __('Comments on old posts are disabled.', 'wp-plugin-bigrock')
			: __('Comments are allowed on old posts.', 'wp-plugin-bigrock');
	};
...
const CloseCommentsDays = ({ setError, notify }) => {
	...
	const closeCommentsDaysNoticeTitle = () => {
		return (
			__('Comments setting saved ', 'wp-plugin-bigrock')
		);
	};
	...
	const closeCommentsDaysNoticeText = () => {
		...
		return (
			__('Comments on posts are disabled after ', 'wp-plugin-bigrock') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-bigrock')
		);
	};
	...
	const closeCommentsDaysLabelText = () => {
		...
		return (
			__('Close comments after ', 'wp-plugin-bigrock') +
			closeCommentsDays +
			_n(' day.', ' days.', parseInt(closeCommentsDays), 'wp-plugin-bigrock')
		);
	};
...
const CommentsPerPage = ({ setError, notify }) => {
	...
	const commentsPerPageNoticeTitle = () => {
		return __('Comments setting saved.', 'wp-plugin-bigrock');
	};

	const commentsPerPageNoticeText = () => {
		//`Posts will display ${commentsPerPage} comments at a time.`
		return (
			__('Posts will display ', 'wp-plugin-bigrock') +
			commentsPerPage +
			_n(
				' comment at a time.',
				' comments at a time.',
	...
	return (
		<SelectField
			...
			label={__('Display ', 'wp-plugin-bigrock') + commentsPerPage +
				__(' comments per page.', 'wp-plugin-bigrock')}
...
const CommentSettings = () => {
	...
	return (
		<Container.SettingsField
			title={__('Comments', 'wp-plugin-bigrock')}
			description={__('Comments allow visitors to provide feedback and respond to your posts or pages.', 'wp-plugin-bigrock')}
		>
