...
const PerformanceFeatureSettings = () => {
	...
	const getPerformanceNoticeTitle = () => {
		return performance
			? __( 'Performance Enabled', 'wp-plugin-bigrock' )
			: __( 'Performance Disabled', 'wp-plugin-bigrock' );
	};
	...
	const getPerformanceNoticeText = () => {
		return performance
			...
			: __( 'Performance will no longer display.', 'wp-plugin-bigrock' );
	};
	...
	const notifyError = () => {
		notify.push( 'feature-toggle-notice', {
			title: __( 'Sorry, that is not allowed.', 'wp-plugin-bigrock' ),
			description: __(
	...
	useUpdateEffect( () => {
		...
		updateUI( '.wppbr-app-navitem-Performance', performance );
	}, [ performance ] );
