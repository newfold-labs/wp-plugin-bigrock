...
export const SideNavMenu = () => {
	...
	const SubMenusManager = () => {
		...
		const subMenus = document.querySelectorAll('.wppbr-app-navitem-submenu');
		subMenus.forEach((subMenu) => {
			subMenu.classList.add('nfd-hidden');
		});

		// open active's submenu if it exists
		const activeMenu = document.querySelector('.wppbr-app-sidenav .active');
		if (
			activeMenu && 
			null !== activeMenu.nextSibling && 
			activeMenu.nextSibling.classList.contains('wppbr-app-navitem-submenu')
		) {
	...
	return (
		<li className="nfd-mb-0">
			...
			{subItems && subItems.length > 0 &&
				<ul className="wppbr-app-navitem-submenu nfd-hidden nfd-ml-8 nfd-m-2 nfd-space-y-1.5">
...
export const SideNav = () => {
	...
	return (
		<aside className="wppbr-app-sidenav nfd-shrink-0 nfd-hidden min-[783px]:nfd-block nfd-pb-6 nfd-bottom-0 nfd-w-56">
			<SidebarNavigation>
		...
		<header className="nfd-sticky nfd-z-30 nfd-top-0 min-[600px]:nfd-top-[46px] nfd-border-b nfd-border-line">
			<div className="nfd-flex nfd-justify-between nfd-items-center nfd-bg-white">
				...
				<Modal
					...
					className="wppbr-app-sidenav-mobile nfd-z-40"
					initialFocus
				>
					<Modal.Panel className="nfd-p-0 nfd-overflow-visible">
						<div className="wppbr-app-sidenav nfd-p-5 nfd-max-h-[70vh] nfd-overflow-y-auto">
							<SideNavMenu />
						</div>
					</Modal.Panel>
				</Modal>
