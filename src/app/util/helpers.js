...
export const webSettingsApiFetch = ( data, passError, thenCallback ) => {
	return apiFetch( {
		// path: 'bigrock/v1/settings', //  can't use path bacause it breaks on temp domains
		url: NewfoldRuntime.createApiUrl( '/bigrock/v1/settings' ),
		method: 'POST',
		data,
	} )
		.then( ( response ) => {
...
export const webPurgeCacheApiFetch = ( data, passError, thenCallback ) => {
	return apiFetch( {
		url: NewfoldRuntime.createApiUrl( '/bigrock/v1/caching' ),
		method: 'DELETE',
		data,
	} )
		.then( ( response ) => {
