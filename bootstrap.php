...
if ( is_readable( __DIR__ . '/vendor/autoload.php' ) ) {
	...
	if ( 'local' === wp_get_environment_type() ) {
		wp_die( esc_html( __( 'Please install the BigRock Plugin dependencies.', 'wp-plugin-bigrock' ) ) );
	}
	return;
}
 ...
 */
$bigrock_module_container = new Container(
	array(
...
$bigrock_module_container->set(
	'plugin',
	$bigrock_module_container->service(
		function () {
			return new Plugin(
				array(
		...
		$args = wp_parse_args(
			array(
				...
				/* translators: %1$s is replaced with opening link tag taking you to bigrock.com/wordpress-hosting, %2$s is replaced with closing link tag, %3$s is replaced with opening link tag taking you to login page, %4$s is replaced with closing link tag, %5$s is replaced with opening link tag taking you to my.bigrock.com, %6$s is replaced with closing link tag */
					...
					'<a href="' . esc_url( 'https://www.bigrock.in/wordpress-hosting/' ) . '" target="_blank" rel="noopener noreferrer nofollow">',strtoupper($brandName),strtoupper($brandName),
					'</a>',
					'<a href="' . esc_url( wp_login_url() ) . '">',
					'</a>',
					'<a href="' . esc_url( 'https://www.bigrock.in/login/' ) . '" target="_blank" rel="noopener noreferrer nofollow">',
					'</a>'
				),
				'template_page_title' => sprintf(
				/* translators: %s: Blog name */
					__( '%s &mdash; Coming Soon', 'wp-plugin-bigrock' ),
					esc_html( get_option( 'blogname' ) )
				),
				'admin_bar_text'      => '<div style="background-color: #FEC101; color: #000; padding: 0 1rem;">' . __( 'Coming Soon Active', 'wp-plugin-bigrock' ) . '</div>',
...
$pluginUpdater = new PluginUpdater( BIGROCK_PLUGIN_FILE, $updateurl );
...
$pluginUpdater->setDataOverrides(
	array(
		'banners' => array(
			'2x' => 'https://cdn.hiive.space/marketplace/vendors-assets/bigrock-banner.svg',
			'1x' => 'https://cdn.hiive.space/marketplace/vendors-assets/bigrock-banner.svg',
		),
		'icons'   => array(
			'2x' => 'https://cdn.hiive.space/marketplace/vendors-assets/bigrock-icon.svg',
			'1x' => 'https://cdn.hiive.space/marketplace/vendors-assets/bigrock-icon.svg',
		),
	)
);
...
if ( is_admin() ) {
	...
	if ( $did_upgrade ) {
		...
		update_option( 'bigrock_plugin_version', BIGROCK_PLUGIN_VERSION, true );
	}
}

// Required files
require BIGROCK_PLUGIN_DIR . '/inc/Admin.php';
require BIGROCK_PLUGIN_DIR . '/inc/AdminBar.php';
require BIGROCK_PLUGIN_DIR . '/inc/base.php';
require BIGROCK_PLUGIN_DIR . '/inc/jetpack.php';
require BIGROCK_PLUGIN_DIR . '/inc/partners.php';
require BIGROCK_PLUGIN_DIR . '/inc/RestApi/CachingController.php';
require BIGROCK_PLUGIN_DIR . '/inc/RestApi/SettingsController.php';
