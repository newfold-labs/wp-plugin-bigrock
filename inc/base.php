 ...
 * @return bool
 */
function bigrock_has_plugin_install_date() {
	return ! empty( get_option( 'bigrock_plugin_install_date', '' ) );
}
 ...
 * @return string
 */
function bigrock_get_plugin_install_date() {
	return (string) get_option( 'bigrock_plugin_install_date', gmdate( 'U' ) );
}
 ...
 */
function bigrock_set_plugin_install_date( $value ) {
	update_option( 'bigrock_plugin_install_date', $value, true );
}
 ...
 * @return int
 */
function bigrock_get_days_since_plugin_install_date() {
	return absint( ( gmdate( 'U' ) - bigrock_get_plugin_install_date() ) / DAY_IN_SECONDS );
}

/**
 * Basic setup
 */
function bigrock_setup() {
	...
	if ( ! bigrock_has_plugin_install_date() ) {
		$date = false;
		if ( ! empty( $install_date ) ) {
			$date = \DateTime::createFromFormat( 'M d, Y', $install_date );
		}
		bigrock_set_plugin_install_date( $date ? $date->format( 'U' ) : gmdate( 'U' ) );
	}
}

add_action( 'admin_init', __NAMESPACE__ . '\\bigrock_setup' );
 ...
 * @return int
 */
function bigrock_install_date_filter( $install_date ) {
	return bigrock_get_plugin_install_date();
}
add_filter( 'nfd_install_date_filter', __NAMESPACE__ . '\\bigrock_install_date_filter' );
